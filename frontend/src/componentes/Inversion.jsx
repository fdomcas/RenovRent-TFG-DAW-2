...
export default function ModalInversion({ inmueble, onClose }) {
  const [step, setStep] = useState(1) // 1=cantidad, 2=pago, 3=confirmado
      ...
      <div style={s.modal}>
          ...
          <div style={s.steps}>
            {['Cantidad', 'Pago', 'Confirmación'].map((label, i) => (
              <div key={i} style={s.stepItem}>
                ...
                <span style={{ ...s.stepLabel, color: step === i + 1 ? '#F97316' : '#aaa' }}>{label}</span>
              </div>
            ))}
          </div>
        )}
            ...
            <div style={s.grid2}>
              <div style={s.seccion}>
                ...
                <div style={{ ...s.infoGrid, marginTop: '1rem' }}>
                  <div style={s.infoItem}>
                    <span style={s.infoLabel}>Ubicación</span>
                    <span style={s.infoVal}>{inmueble.ubicacion}</span>
                  </div>
                  <div style={s.infoItem}>
                    <span style={s.infoLabel}>Valor inmueble</span>
                    <span style={s.infoVal}>{inmueble.precio?.toLocaleString('es-ES')}€</span>
                  </div>
                  <div style={s.infoItem}>
                    <span style={s.infoLabel}>Rentabilidad esperada</span>
                    <span style={s.infoVal}>{inmueble.retorno_anual_porcentaje}% anual</span>
                  </div>
                  <div style={s.infoItem}>
                    <span style={s.infoLabel}>Pisos alquilados</span>
                    <span style={s.infoVal}>{inmueble.num_habitaciones || '—'}</span>
                  </div>
                </div>
              </div>
              ...
              <div style={s.resumen}>
                ...
                <div style={s.resumenFila}>
                  <span>Capital a invertir</span>
                  <strong>{cantidad.toLocaleString('es-ES')}€</strong>
                </div>
                <div style={s.resumenFila}>
                ...
                <div style={s.resumenFila}>
                  ...
                  <strong style={{ color: '#16a34a' }}>{retornoMensual.toLocaleString('es-ES')}€</strong>
                </div>
                ...
                <button style={s.btnNaranja} onClick={() => setStep(2)}>
                  Invertir →
                </button>
              </div>
            </div>
          ...
          <div style={s.body}>
            <div style={s.grid2}>
              <div style={s.seccion}>
                ...
                <div style={s.metodosWrap}>
                  <button
                    style={{ ...s.metodoBtn, ...(metodo === 'tarjeta' ? s.metodoBtnActivo : {}) }}
                    onClick={() => setMetodo('tarjeta')}
                  >
                    💳 Tarjeta crédito/débito
                  </button>
                  <button
                    style={{ ...s.metodoBtn, ...(metodo === 'transferencia' ? s.metodoBtnActivo : {}) }}
                  ...
                  <div style={{ marginTop: '1rem' }}>
                    {tarjetas.length > 0 && !nuevaTarjeta && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '0.8rem' }}>
                        ...
                        <button style={s.btnAddTarjeta} onClick={() => setNuevaTarjeta(true)}>
                          + Usar otra tarjeta
                        </button>
                      </div>
                    )}
                    ...
                    {(nuevaTarjeta || tarjetas.length === 0) && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                        {tarjetas.length > 0 && (
                          <button style={s.btnAddTarjeta} onClick={() => setNuevaTarjeta(false)}>
                        ...
                        <div style={s.fila}>
                          <div style={s.campo}>
                            ...
                            <input style={s.input} value={formTarjeta.nombre_titular} onChange={e => setFormTarjeta({ ...formTarjeta, nombre_titular: e.target.value })} />
                          </div>
                        </div>
                        <div style={s.fila}>
                          <div style={s.campo}>
                            <label style={s.label}>Número de tarjeta</label>
                            <input style={s.input} placeholder="1234 5678 9012 3456" maxLength={19} value={formTarjeta.numero_tarjeta} onChange={e => setFormTarjeta({ ...formTarjeta, numero_tarjeta: e.target.value })} />
                          </div>
                        </div>
                        <div style={s.fila}>
                          <div style={s.campo}>
                            <label style={s.label}>Fecha caducidad</label>
                            <input style={s.input} placeholder="MM/YYYY" value={formTarjeta.fecha_caducidad} onChange={e => setFormTarjeta({ ...formTarjeta, fecha_caducidad: e.target.value })} />
                          </div>
                        </div>
                      </div>
                    )}
                ...
                {metodo === 'transferencia' && (
                  <div style={{ ...s.ibanBox, marginTop: '1rem' }}>
                    <p style={{ fontSize: '0.88rem', color: '#555', marginBottom: '0.4rem' }}>
                      Realiza la transferencia a este IBAN:
                    </p>
                    <code style={s.ibanCode}>ES91 2100 0418 4502 0005 1332</code>
                    <p style={{ fontSize: '0.8rem', color: '#888', marginTop: '0.5rem' }}>
                      Concepto: <strong>INV-{perfil?.dni}-{inmueble.id}</strong>
                    </p>
                    <p style={{ fontSize: '0.8rem', color: '#F97316', marginTop: '0.3rem' }}>
                      ⚠️ La inversión se activará al recibir el pago (1-2 días hábiles)
                    </p>
                  </div>
                )}
              ...
              <div style={s.resumen}>
                ...
                <div style={s.resumenFila}>
                  ...
                  <strong style={{ color: '#16a34a' }}>{inmueble.retorno_anual_porcentaje}% anual</strong>
                </div>
                <div style={s.resumenFila}>
        ...
        {step === 3 && (
          <div style={s.confirmado}>
            ...
            <button style={s.btnNaranja} onClick={onClose}>Cerrar</button>
          </div>
        )}
...
const s = {
  ...
  modal:          { background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '900px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' },
  header:         { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '1.5rem 2rem 1rem', borderBottom: '1px solid #f3f4f6' },
  titulo:         { fontSize: '1.2rem', fontWeight: '800', color: '#1a1a1a', margin: 0 },
  subtitulo:      { color: '#888', fontSize: '0.85rem', marginTop: '0.2rem' },
  cerrar:         { background: 'none', border: 'none', fontSize: '1.2rem', cursor: 'pointer', color: '#aaa', padding: '0.2rem' },
  steps:          { display: 'flex', justifyContent: 'center', gap: '3rem', padding: '1rem 2rem', borderBottom: '1px solid #f3f4f6' },
  ...
  grid2:          { display: 'grid', gridTemplateColumns: '1fr 340px', gap: '2rem' },
  ...
  datosGrid:      { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' },
  ...
  confirmado:     { padding: '3rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', textAlign: 'center' },
  ...
  confirmResumen: { background: '#f9f8f5', borderRadius: '10px', padding: '1rem 1.5rem', width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', gap: '0.5rem' },
}